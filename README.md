...
<br><b>Execute given JovianDSS command for automated setup and to control JovianDSS remotely.</b>


 EXAMPLES:

 1. <br><b>Create clone</b> of iSCSI volume zvol00 from Pool-0 and attach to iSCSI target.
 ...
 2. <br><b>Create clone</b> of NAS volume vol00 from Pool-0 and share via new created SMB share.
 ...
 3. <br><b>Delete clone</b> of iSCSI volume zvol00 from Pool-0.

        jdss-api-tools.exe delete_clone --pool Pool-0 --volume zvol00 --node 192.168.0.220

 4. <br><b>Delete clone</b> of NAS volume vol00 from Pool-0.

        jdss-api-tools.exe delete_clone --pool Pool-0 --volume vol00 --node 192.168.0.220

 5. <br><b>Create clone</b> of existing snapshot on iSCSI volume zvol00 from Pool-0 and attach to iSCSI target.
 ...
 6. <br><b>Create clone</b> of existing snapshot on NAS volume vol00 from Pool-0 and share via new created SMB share.
 ...
 7. <br><b>Delete clone</b> of existing snapshot on iSCSI volume zvol00 from Pool-0.
 ...
 8. <br><b>Delete clone</b> of existing snapshot on NAS volume vol00 from Pool-0.
 ...
 9. <br><b>Create pool</b> on single node or cluster with single JBOD:
...
10. <br><b>Create pool</b> on Metro Cluster with single JBOD with 4-way mirrors:
...
11. <br><b>Create pool</b> with raidz2(4 disks each) over 4 JBODs with 60 HDD each.
...
12. <br><b>Shutdown</b> three JovianDSS servers using default port but non default password.
...
13. <br><b>Reboot</b> single JovianDSS server.

        jdss-api-tools.exe reboot --node 192.168.0.220

14. <br><b>Set host name</b> to "node220", server name to "server220" and server description to "jdss220".

        jdss-api-tools.exe set_host --host node220 --server server220 --description jdss220 --node 192.168.0.220

15. <br><b>Set timezone and NTP-time</b> with default NTP servers.
...
16. <br><b>Set new IP settings</b> for eth0 and set gateway-IP and set eth0 as default gateway.
...
17. <br><b>Create bond</b> examples. Bond types: balance-rr, active-backup.
...
18. <br><b>Delete bond</b>.

        jdss-api-tools.exe delete_bond --nic bond0 --node 192.168.0.80

19. <br><b>Bind cluster</b>. Bind node-b: 192.168.0.81 with node-a: 192.168.0.80
...
20. <br><b>Set HA-cluster ping nodes</b>. First IP   access node IP, next IPs are new ping nodes
...
21. <br><b>Set HA-cluster mirror path</b>. Please enter comma separated NICs, the first NIC must be from the same node as the specified access IP.

        jdss-api-tools.exe set_mirror_path --mirror_nics eth4 eth4 --node 192.168.0.82

22. <br><b>Create VIP (Virtual IP)</b> examples. 
...
23. <br><b>Start HA-cluster</b>. Please enter first node IP address only.

        jdss-api-tools.exe start_cluster --node 192.168.0.82

24. <br><b>Move (failover)</b> given pool.
...
25. <br><b>Create storage resource</b>. Creates iSCSI target with volume or SMB share with dataset.
...
28. <br><b>Genarate factory setup files for batch setup.</b>.
...
29. <br><b>Execute factory setup files for batch setup.</b>.
